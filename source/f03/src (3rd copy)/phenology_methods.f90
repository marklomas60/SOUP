...
if (ssv(co)%no_suma_comps > 0) then
  if (ssp%jday-int(ssv(co)%suma_comps(1,2)) > 360) then
    k = suma_comp_length-(ssp%jday-int(ssv(co)%suma_comps(1,2))-360)+1
    if (k == 1) then
      do i=1,ssv(co)%no_suma_comps-1
        ssv(co)%suma_comps(i,1) = ssv(co)%suma_comps(i+1,1) 
        ssv(co)%suma_comps(i,2) = ssv(co)%suma_comps(i+1,2) 
      enddo
      ssv(co)%suma_comps(ssv(co)%no_suma_comps,1) = 0.0
      ssv(co)%suma_comps(ssv(co)%no_suma_comps,2) = 0.0
      ssv(co)%no_suma_comps = ssv(co)%no_suma_comps - 1
    else
      ssv(co)%suma_comps(1,1) = ssv(co)%suma_comps(1,1)*(real(k-1)/real(k))
    endif
  endif
endif
...
if (laiinc>0.0) then
  if (ssv(co)%no_suma_comps == 0) then
    ...
    ssv(co)%suma_comps(ssv(co)%no_suma_comps,1) = 0.0
    ssv(co)%suma_comps(ssv(co)%no_suma_comps,2) = ssp%jday
  elseif (ssp%jday-int(ssv(co)%suma_comps(ssv(co)%no_suma_comps,2)) >= suma_comp_length) then
    ssv(co)%no_suma_comps = ssv(co)%no_suma_comps + 1
    ssv(co)%suma_comps(ssv(co)%no_suma_comps,1) = 0.0
    ssv(co)%suma_comps(ssv(co)%no_suma_comps,2) = ssp%jday
  endif
  ssv(co)%suma_comps(ssv(co)%no_suma_comps,1) =  ssv(co)%suma_comps(ssv(co)%no_suma_comps,1) + laiinc 
endif

ssv(co)%suma = 0.0
do i=1,ssv(co)%no_suma_comps
  ssv(co)%suma = ssv(co)%suma + ssv(co)%suma_comps(i,1)
enddo
