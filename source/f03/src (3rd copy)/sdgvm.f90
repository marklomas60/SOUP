...
!----------------------------------------------------------------------!
     ...
     s1in,tmp(mnth,day),ssv(ft)%lai,evap,tran,roff,interc,evbs,f2,f3,ft)
...
!----------------------------------------------------------------------!
            daily_out(1,ft,mnth,day) = ssv(ft)%lai
...
!        daily_out(23,ft,mnth,day) = leaflit(ft) - lflitold
 ...
 ssv(ft)%lai*12.0/pft(ft)%sla/18.0
            daily_out(27,ft,mnth,day) = &
 ssv(ft)%bio(1) + ssv(ft)%stem + ssv(ft)%nppstore(1)
            daily_out(28,ft,mnth,day) = ssv(ft)%bio(2) + ssv(ft)%root
