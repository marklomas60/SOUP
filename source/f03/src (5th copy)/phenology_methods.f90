...
if (check_closure) then
  old_total_carbon = ssv(co)%lai*12.0/pft(co)%sla/18.0 + ssv(co)%nppstore(1) + &
 ssv(co)%stem + ssv(co)%root + resp + ssv(co)%bio(1) + ssv(co)%bio(2)
endif
...
!----------------------------------------------------------------------!
if (check_closure) then
  ans = ssv(co)%lai*12.0/pft(co)%sla/18.0 + ssv(co)%nppstore(1) + &
 ssv(co)%stem + ssv(co)%root + resp + ssv(co)%bio(1) + ssv(co)%bio(2)
...
!----------------------------------------------------------------------!
if (check_closure) then
  total_carbon = ssv(co)%lai*12.0/pft(co)%sla/18.0 + ssv(co)%nppstore(1) + &
 ssv(co)%stem + ssv(co)%root + resp + ssv(co)%bio(1) + ssv(co)%bio(2)
...
!----------------------------------------------------------------------!
if (ssv(co)%no_suma_comps > 0) then
  if (ssp%jday-int(ssv(co)%suma_comps(1)%age) > 360) then
    k = suma_comp_length-(ssp%jday-int(ssv(co)%suma_comps(1)%age)-360)+1
    if (k == 1) then
      do i=1,ssv(co)%no_suma_comps-1
        ssv(co)%suma_comps(i)%val = ssv(co)%suma_comps(i+1)%val
        ssv(co)%suma_comps(i)%age = ssv(co)%suma_comps(i+1)%age
      enddo
      ssv(co)%suma_comps(ssv(co)%no_suma_comps)%val = 0.0
      ssv(co)%suma_comps(ssv(co)%no_suma_comps)%age = 0.0
      ssv(co)%no_suma_comps = ssv(co)%no_suma_comps - 1
    else
      ssv(co)%suma_comps(1)%val = ssv(co)%suma_comps(1)%val*(real(k-1)/real(k))
    endif
  endif
endif
...
if (laiinc>0.0) then
  if (ssv(co)%no_suma_comps == 0) then
    ssv(co)%no_suma_comps = 1
    ssv(co)%suma_comps(ssv(co)%no_suma_comps)%val = 0.0
    ssv(co)%suma_comps(ssv(co)%no_suma_comps)%age = ssp%jday
  elseif (ssp%jday-int(ssv(co)%suma_comps(ssv(co)%no_suma_comps)%age) >= suma_comp_length) then
    ssv(co)%no_suma_comps = ssv(co)%no_suma_comps + 1
    ssv(co)%suma_comps(ssv(co)%no_suma_comps)%val = 0.0
    ssv(co)%suma_comps(ssv(co)%no_suma_comps)%age = ssp%jday
  endif
  ssv(co)%suma_comps(ssv(co)%no_suma_comps)%val =  ssv(co)%suma_comps(ssv(co)%no_suma_comps)%val + laiinc
endif

ssv(co)%suma = 0.0
do i=1,ssv(co)%no_suma_comps
  ssv(co)%suma = ssv(co)%suma + ssv(co)%suma_comps(i)%val
enddo
...
!----------------------------------------------------------------------!
if (ssv(co)%no_stem_comps > 0) then
  if (ssp%jday-int(ssv(co)%stem_comps(1)%age) > pft(co)%sls) then
    k = stem_comp_length-(ssp%jday-int(ssv(co)%stem_comps(1)%age)-pft(co)%sls)+1
    if (k == 1) then
      stem_fixed = ssv(co)%stem_comps(1)%val
      do i=1,ssv(co)%no_stem_comps-1
        ssv(co)%stem_comps(i)%val = ssv(co)%stem_comps(i+1)%val
        ssv(co)%stem_comps(i)%age = ssv(co)%stem_comps(i+1)%age
      enddo
      ssv(co)%stem_comps(ssv(co)%no_stem_comps)%val = 0.0
      ssv(co)%stem_comps(ssv(co)%no_stem_comps)%age = 0.0
      ssv(co)%no_stem_comps = ssv(co)%no_stem_comps - 1
    else
      stem_fixed = ssv(co)%stem_comps(1)%val/real(k)
      ssv(co)%stem_comps(1)%val = ssv(co)%stem_comps(1)%val*(real(k-1)/real(k))
    endif
  endif
endif
...
if (stem_inc>0.0) then
  if (ssv(co)%no_stem_comps == 0) then
    ssv(co)%no_stem_comps = 1
    ssv(co)%stem_comps(ssv(co)%no_stem_comps)%val = 0.0
    ssv(co)%stem_comps(ssv(co)%no_stem_comps)%age = ssp%jday
  elseif (ssp%jday-int(ssv(co)%stem_comps(ssv(co)%no_stem_comps)%age) >= stem_comp_length) then
    ssv(co)%no_stem_comps = ssv(co)%no_stem_comps + 1
    ssv(co)%stem_comps(ssv(co)%no_stem_comps)%val = 0.0
    ssv(co)%stem_comps(ssv(co)%no_stem_comps)%age = ssp%jday
  endif
  ssv(co)%stem_comps(ssv(co)%no_stem_comps)%val =  ssv(co)%stem_comps(ssv(co)%no_stem_comps)%val + stem_inc
endif
...
! Leaf death through old age.
!----------------------------------------------------------------------!
if (ssv(co)%no_root_comps > 0) then
  if (ssp%jday-int(ssv(co)%root_comps(1)%age) > pft(co)%rls) then
    k = root_comp_length-(ssp%jday-int(ssv(co)%root_comps(1)%age)-pft(co)%rls)+1
    if (k == 1) then
      leaflit = ssv(co)%root_comps(1)%val
      do i=1,ssv(co)%no_root_comps-1
        ssv(co)%root_comps(i)%val = ssv(co)%root_comps(i+1)%val
        ssv(co)%root_comps(i)%age = ssv(co)%root_comps(i+1)%age
      enddo
      ssv(co)%root_comps(ssv(co)%no_root_comps)%val = 0.0
      ssv(co)%root_comps(ssv(co)%no_root_comps)%age = 0.0
      ssv(co)%no_root_comps = ssv(co)%no_root_comps - 1
    else
      leaflit = ssv(co)%root_comps(1)%val/real(k)
      ssv(co)%root_comps(1)%val = ssv(co)%root_comps(1)%val*&
 (real(k-1)/real(k))
    endif
  endif
endif
...
if (laiinc>0.0) then
  if (ssv(co)%no_root_comps == 0) then
    ssv(co)%no_root_comps = 1
    ssv(co)%root_comps(ssv(co)%no_root_comps)%val = 0.0
    ssv(co)%root_comps(ssv(co)%no_root_comps)%age = ssp%jday
  elseif (ssp%jday-int(ssv(co)%root_comps(ssv(co)%no_root_comps)%age) &
 >= root_comp_length) then
    ssv(co)%no_root_comps = ssv(co)%no_root_comps + 1
    ssv(co)%root_comps(ssv(co)%no_root_comps)%val = 0.0
    ssv(co)%root_comps(ssv(co)%no_root_comps)%age = ssp%jday
  endif
  ssv(co)%root_comps(ssv(co)%no_root_comps)%val =  &
 ssv(co)%root_comps(ssv(co)%no_root_comps)%val + laiinc
endif
...
! Leaf death through old age.
!----------------------------------------------------------------------!
if (ssv(co)%no_lai_comps > 0) then
  if (ssp%jday-int(ssv(co)%lai_comps(1)%age) > pft(co)%lls) then
    k = lai_comp_length-(ssp%jday-int(ssv(co)%lai_comps(1)%age)-pft(co)%lls)+1
    if (k == 1) then
      leaflit = ssv(co)%lai_comps(1)%val
      do i=1,ssv(co)%no_lai_comps-1
        ssv(co)%lai_comps(i)%val = ssv(co)%lai_comps(i+1)%val
        ssv(co)%lai_comps(i)%age = ssv(co)%lai_comps(i+1)%age
      enddo
      ssv(co)%lai_comps(ssv(co)%no_lai_comps)%val = 0.0
      ssv(co)%lai_comps(ssv(co)%no_lai_comps)%age = 0.0
      ssv(co)%no_lai_comps = ssv(co)%no_lai_comps - 1
    else
      leaflit = ssv(co)%lai_comps(1)%val/real(k)
      ssv(co)%lai_comps(1)%val = ssv(co)%lai_comps(1)%val*(real(k-1)/real(k))
    endif
  endif
endif
...
if (laiinc>0.0) then
  if (ssv(co)%no_lai_comps == 0) then
    ssv(co)%no_lai_comps = 1
    ssv(co)%lai_comps(ssv(co)%no_lai_comps)%val = 0.0
    ssv(co)%lai_comps(ssv(co)%no_lai_comps)%age = ssp%jday
  elseif (ssp%jday-int(ssv(co)%lai_comps(ssv(co)%no_lai_comps)%age) >= lai_comp_length) then
    ssv(co)%no_lai_comps = ssv(co)%no_lai_comps + 1
    ssv(co)%lai_comps(ssv(co)%no_lai_comps)%val = 0.0
    ssv(co)%lai_comps(ssv(co)%no_lai_comps)%age = ssp%jday
  endif
  ssv(co)%lai_comps(ssv(co)%no_lai_comps)%val =  ssv(co)%lai_comps(ssv(co)%no_lai_comps)%val + laiinc
...
do i=1,ssv(co)%no_stem_comps
  ans = ssv(co)%stem_comps(i)%val*respref
  resp = resp + ans
  ssv(co)%stem_comps(i)%val = ssv(co)%stem_comps(i)%val - ans
enddo

ssv(co)%stem = 0.0
do i=1,ssv(co)%no_stem_comps
  ssv(co)%stem = ssv(co)%stem + ssv(co)%stem_comps(i)%val
enddo
...
do i=1,ssv(co)%no_root_comps
  ans = ssv(co)%root_comps(i)%val*respref
  resp = resp + ans
  ssv(co)%root_comps(i)%val = ssv(co)%root_comps(i)%val - ans
enddo

ssv(co)%root = 0.0
do i=1,ssv(co)%no_root_comps
  ssv(co)%root = ssv(co)%root + ssv(co)%root_comps(i)%val
enddo
...
do i=1,ssv(ssp%cohort)%no_lai_comps
  ans = lmor_sc(int(ssp%jday-ssv(co)%lai_comps(i)%age+1.5))*0.0+1.0
  leaflit = leaflit + ssv(co)%lai_comps(i)%val*(1.0 - ans)
  ssv(co)%lai_comps(i)%val = ssv(co)%lai_comps(i)%val*ans
enddo

! Sum up the lai compartments.
ssv(co)%lai = 0.0
do i=1,ssv(co)%no_lai_comps
  ssv(co)%lai = ssv(co)%lai + ssv(co)%lai_comps(i)%val
enddo
