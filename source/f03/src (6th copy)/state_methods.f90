...
oldestx = ssp%jday
if (ssv(co1)%no_lai_comps > 0) oldestx = min(oldestx,ssv(co1)%lai_comps(1)%age)
if (ssv(co2)%no_lai_comps > 0) oldestx = min(oldestx,ssv(co2)%lai_comps(1)%age)
...
do i=1,ssv(co1)%no_lai_comps
  do k=1,lai_comp_length
    j = int(ssv(co1)%lai_comps(i)%age - oldestx + 0.5) + k
    j = min(j,ssp%jday-int(oldestx)+1)
    comps(j) = comps(j) + ssv(co1)%lai_comps(i)%val*t1/real(lai_comp_length)
  enddo
enddo

do i=1,ssv(co2)%no_lai_comps
  do k=1,lai_comp_length
    j = int(ssv(co2)%lai_comps(i)%age - oldestx + 0.5) + k
    j = min(j,ssp%jday-int(oldestx)+1)
    comps(j) = comps(j) + ssv(co2)%lai_comps(i)%val*t2/real(lai_comp_length)
  enddo
enddo
...
do i=1,max_lai_comps
  ...
  if (ans > 0.0) then
    ...
    ssv_temp%lai_comps(ssv_temp%no_lai_comps)%val = ans
    ssv_temp%lai_comps(ssv_temp%no_lai_comps)%age = real((i-1)*lai_comp_length) + oldestx
  endif
enddo
...
!----------------------------------------------------------------------*
oldestx = ssp%jday
if (ssv(co1)%no_stem_comps > 0) oldestx = min(oldestx,ssv(co1)%stem_comps(1)%age)
if (ssv(co2)%no_stem_comps > 0) oldestx = min(oldestx,ssv(co2)%stem_comps(1)%age)
...
do i=1,ssv(co1)%no_stem_comps
  do k=1,stem_comp_length
    j = int(ssv(co1)%stem_comps(i)%age - oldestx + 0.5) + k
    comps(j) = comps(j) + ssv(co1)%stem_comps(i)%val*t1/real(stem_comp_length)
  enddo
enddo
do i=1,ssv(co2)%no_stem_comps
  do k=1,stem_comp_length
    j = int(ssv(co2)%stem_comps(i)%age - oldestx + 0.5) + k
    comps(j) = comps(j) + ssv(co2)%stem_comps(i)%val*t2/real(stem_comp_length)
  enddo
enddo
do i=1,max_stem_comps*stem_comp_length
  comps(i) = comps(i)/t12
...
do i=1,max_stem_comps
  ...
  if (ans > 0.0) then
    ...
    ssv_temp%stem_comps(ssv_temp%no_stem_comps)%val = ans
    ssv_temp%stem_comps(ssv_temp%no_stem_comps)%age = real((i-1)*stem_comp_length) + oldestx
  endif
enddo
...
!----------------------------------------------------------------------*
!oldest = ssp%jday
!if (ssv(co1)%no_root_comps > 0) oldest = min(oldest,real(ssv(co1)%root_comps(1)%age))
!if (ssv(co2)%no_root_comps > 0) oldest = min(oldest,real(ssv(co2)%root_comps(1)%age))

oldestx = ssp%jday
if (ssv(co1)%no_root_comps > 0) oldestx = min(ssp%jday,ssv(co1)%root_comps(1)%age)
if (ssv(co2)%no_root_comps > 0) oldestx = min(oldestx,ssv(co2)%root_comps(1)%age)

!if (int(oldest).ne.oldestx) then
!  print*,'oldest old new ',int(oldest),oldestx
!  print*,ssv(co1)%root_comps(1)%age,ssv(co2)%root_comps(1)%age
!  print*,ssp%jday
!endif
...
do i=1,ssv(co1)%no_root_comps
  do k=1,root_comp_length
    j = int(ssv(co1)%root_comps(i)%age - oldestx + 0.5) + k
    comps(j) = comps(j) + ssv(co1)%root_comps(i)%val*t1/real(root_comp_length)
  enddo
enddo
do i=1,ssv(co2)%no_root_comps
  do k=1,root_comp_length
    j = int(ssv(co2)%root_comps(i)%age - oldestx + 0.5) + k
    comps(j) = comps(j) + ssv(co2)%root_comps(i)%val*t2/real(root_comp_length)
  enddo
enddo
do i=1,max_root_comps*root_comp_length
  comps(i) = comps(i)/t12
...
do i=1,max_root_comps
  ...
  if (ans > 0.0) then
    ...
    ssv_temp%root_comps(ssv_temp%no_root_comps)%val = ans
    ssv_temp%root_comps(ssv_temp%no_root_comps)%age = real((i-1)*root_comp_length) + oldestx
  endif
enddo
...
!----------------------------------------------------------------------*
oldestx = ssp%jday
if (ssv(co1)%no_suma_comps > 0) oldestx = min(oldestx,ssv(co1)%suma_comps(1)%age)
if (ssv(co2)%no_suma_comps > 0) oldestx = min(oldestx,ssv(co2)%suma_comps(1)%age)
...
do i=1,ssv(co1)%no_suma_comps
  do k=1,suma_comp_length
    j = int(ssv(co1)%suma_comps(i)%age - oldestx + 0.5) + k
    comps(j) = comps(j) + ssv(co1)%suma_comps(i)%val*t1/real(suma_comp_length)
  enddo
enddo
do i=1,ssv(co2)%no_suma_comps
  do k=1,suma_comp_length
    j = int(ssv(co2)%suma_comps(i)%age - oldestx + 0.5) + k
    comps(j) = comps(j) + ssv(co2)%suma_comps(i)%val*t2/real(suma_comp_length)
  enddo
enddo
do i=1,max_suma_comps*suma_comp_length
  comps(i) = comps(i)/t12
...
do i=1,max_suma_comps
  ...
  if (ans > 0.0) then
    ...
    ssv_temp%suma_comps(ssv_temp%no_suma_comps)%val = ans
    ssv_temp%suma_comps(ssv_temp%no_suma_comps)%age = real((i-1)*suma_comp_length) + oldestx
  endif
enddo
...
do i=1,int(pft(cohort)%lls/lai_comp_length)+1
  ssv(cohort)%lai_comps(i)%val = 0.0
  ssv(cohort)%lai_comps(i)%age = 0.0
enddo
ssv(cohort)%no_lai_comps = 0
ssv(cohort)%lai = 0.0

do i=1,int(pft(cohort)%sls/stem_comp_length)+1
  ssv(cohort)%stem_comps(i)%val = 0.0
  ssv(cohort)%stem_comps(i)%age = 0.0
enddo
ssv(cohort)%no_stem_comps = 0
ssv(cohort)%stem = 0.0

do i=1,int(pft(cohort)%rls/root_comp_length)+1
  ssv(cohort)%root_comps(i)%val = 0.0
  ssv(cohort)%root_comps(i)%age = 0
enddo
ssv(cohort)%no_root_comps = 0
ssv(cohort)%root = 0.0

do i=1,int(360/suma_comp_length)+1
  ssv(cohort)%suma_comps(i)%val = 0.0
  ssv(cohort)%suma_comps(i)%age = 0.0
enddo
