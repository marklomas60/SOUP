...
if (ssv(co)%no_stem_comps > 0) then
  if (ssp%jday-int(ssv(co)%stem_comps(1,2)) > pft(co)%sls) then
    k = stem_comp_length-(ssp%jday-int(ssv(co)%stem_comps(1,2))-pft(co)%sls)+1
    if (k == 1) then
      stem_fixed = ssv(co)%stem_comps(1,1)
      do i=1,ssv(co)%no_stem_comps-1
        ssv(co)%stem_comps(i,1) = ssv(co)%stem_comps(i+1,1) 
        ssv(co)%stem_comps(i,2) = ssv(co)%stem_comps(i+1,2) 
      enddo
      ssv(co)%stem_comps(ssv(co)%no_stem_comps,1) = 0.0
      ssv(co)%stem_comps(ssv(co)%no_stem_comps,2) = 0.0
      ssv(co)%no_stem_comps = ssv(co)%no_stem_comps - 1
    else
      stem_fixed = ssv(co)%stem_comps(1,1)/real(k)
      ssv(co)%stem_comps(1,1) = ssv(co)%stem_comps(1,1)*(real(k-1)/real(k))
    endif
  endif
endif
...
if (stem_inc>0.0) then
  if (ssv(co)%no_stem_comps == 0) then
    ...
    ssv(co)%stem_comps(ssv(co)%no_stem_comps,1) = 0.0
    ssv(co)%stem_comps(ssv(co)%no_stem_comps,2) = ssp%jday
  elseif (ssp%jday-int(ssv(co)%stem_comps(ssv(co)%no_stem_comps,2)) >= stem_comp_length) then
    ssv(co)%no_stem_comps = ssv(co)%no_stem_comps + 1
    ssv(co)%stem_comps(ssv(co)%no_stem_comps,1) = 0.0
    ssv(co)%stem_comps(ssv(co)%no_stem_comps,2) = ssp%jday
  endif
  ssv(co)%stem_comps(ssv(co)%no_stem_comps,1) =  ssv(co)%stem_comps(ssv(co)%no_stem_comps,1) + stem_inc
endif
...
do i=1,ssv(co)%no_stem_comps
  ans = ssv(co)%stem_comps(i,1)*respref
  resp = resp + ans
  ssv(co)%stem_comps(i,1) = ssv(co)%stem_comps(i,1) - ans
enddo

ssv(co)%stem = 0.0
do i=1,ssv(co)%no_stem_comps
  ssv(co)%stem = ssv(co)%stem + ssv(co)%stem_comps(i,1)
enddo
