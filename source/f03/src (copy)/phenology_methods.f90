...
if (ssv(co)%no_root_comps > 0) then
  if (ssp%jday-int(ssv(co)%root_comps(1,2)) > pft(co)%rls) then
    k = root_comp_length-(ssp%jday-int(ssv(co)%root_comps(1,2))-pft(co)%rls)+1
    if (k == 1) then
      leaflit = ssv(co)%root_comps(1,1)
      do i=1,ssv(co)%no_root_comps-1
        ssv(co)%root_comps(i,1) = ssv(co)%root_comps(i+1,1) 
        ssv(co)%root_comps(i,2) = ssv(co)%root_comps(i+1,2) 
      enddo
      ssv(co)%root_comps(ssv(co)%no_root_comps,1) = 0.0
      ssv(co)%root_comps(ssv(co)%no_root_comps,2) = 0.0
      ssv(co)%no_root_comps = ssv(co)%no_root_comps - 1
    else
      leaflit = ssv(co)%root_comps(1,1)/real(k)
      ssv(co)%root_comps(1,1) = ssv(co)%root_comps(1,1)*&
 (real(k-1)/real(k))
    endif
  endif
endif
...
if (laiinc>0.0) then
  if (ssv(co)%no_root_comps == 0) then
    ...
    ssv(co)%root_comps(ssv(co)%no_root_comps,1) = 0.0
    ssv(co)%root_comps(ssv(co)%no_root_comps,2) = ssp%jday
  elseif (ssp%jday-int(ssv(co)%root_comps(ssv(co)%no_root_comps,2)) &
 >= root_comp_length) then
    ssv(co)%no_root_comps = ssv(co)%no_root_comps + 1
    ssv(co)%root_comps(ssv(co)%no_root_comps,1) = 0.0
    ssv(co)%root_comps(ssv(co)%no_root_comps,2) = ssp%jday
  endif
  ssv(co)%root_comps(ssv(co)%no_root_comps,1) =  &
 ssv(co)%root_comps(ssv(co)%no_root_comps,1) + laiinc
endif
...
do i=1,ssv(co)%no_root_comps
  ans = ssv(co)%root_comps(i,1)*respref
  resp = resp + ans
  ssv(co)%root_comps(i,1) = ssv(co)%root_comps(i,1) - ans
enddo

ssv(co)%root = 0.0
do i=1,ssv(co)%no_root_comps
  ssv(co)%root = ssv(co)%root + ssv(co)%root_comps(i,1)
enddo
