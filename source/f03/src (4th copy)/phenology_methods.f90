...
if (ssv(co)%no_lai_comps > 0) then
  if (ssp%jday-int(ssv(co)%lai_comps(1,2)) > pft(co)%lls) then
    k = lai_comp_length-(ssp%jday-int(ssv(co)%lai_comps(1,2))-pft(co)%lls)+1
    if (k == 1) then
      leaflit = ssv(co)%lai_comps(1,1)
      do i=1,ssv(co)%no_lai_comps-1
        ssv(co)%lai_comps(i,1) = ssv(co)%lai_comps(i+1,1)
        ssv(co)%lai_comps(i,2) = ssv(co)%lai_comps(i+1,2)
      enddo
      ssv(co)%lai_comps(ssv(co)%no_lai_comps,1) = 0.0
      ssv(co)%lai_comps(ssv(co)%no_lai_comps,2) = 0.0
      ssv(co)%no_lai_comps = ssv(co)%no_lai_comps - 1
    else
      leaflit = ssv(co)%lai_comps(1,1)/real(k)
      ssv(co)%lai_comps(1,1) = ssv(co)%lai_comps(1,1)*(real(k-1)/real(k))
    endif
  endif
endif
...
if (laiinc>0.0) then
  if (ssv(co)%no_lai_comps == 0) then
    ...
    ssv(co)%lai_comps(ssv(co)%no_lai_comps,1) = 0.0
    ssv(co)%lai_comps(ssv(co)%no_lai_comps,2) = ssp%jday
  elseif (ssp%jday-int(ssv(co)%lai_comps(ssv(co)%no_lai_comps,2)) >= lai_comp_length) then
    ssv(co)%no_lai_comps = ssv(co)%no_lai_comps + 1
    ssv(co)%lai_comps(ssv(co)%no_lai_comps,1) = 0.0
    ssv(co)%lai_comps(ssv(co)%no_lai_comps,2) = ssp%jday
  endif
  ssv(co)%lai_comps(ssv(co)%no_lai_comps,1) =  ssv(co)%lai_comps(ssv(co)%no_lai_comps,1) + laiinc
...
do i=1,ssv(ssp%cohort)%no_lai_comps
  ans = lmor_sc(int(ssp%jday-ssv(co)%lai_comps(i,2)+1.5))*0.0+1.0
  leaflit = leaflit + ssv(co)%lai_comps(i,1)*(1.0 - ans)
  ssv(co)%lai_comps(i,1) = ssv(co)%lai_comps(i,1)*ans
enddo

! Sum up the lai compartments.
ssv(co)%lai = 0.0
do i=1,ssv(co)%no_lai_comps
  ssv(co)%lai = ssv(co)%lai + ssv(co)%lai_comps(i,1)
enddo
