...
if ((check_closure).and.(pft(co)%sla > 0.0)) then
  old_total_carbon = ssv(co)%lai*12.0/pft(co)%sla/18.0 + &
 ssv(co)%nppstore(1) + ssv(co)%stem%tot + ssv(co)%root + ssv(co)%bio(1) + &
 ssv(co)%bio(2)
endif
!----------------------------------------------------------------------!
...
!----------------------------------------------------------------------!
if ((check_closure).and.(pft(co)%sla > 0.0)) then
  total_carbon = ssv(co)%lai*12.0/pft(co)%sla/18.0 + &
 ssv(co)%nppstore(1) + ssv(co)%stem%tot + ssv(co)%root + ssv(co)%bio(1) + &
 ssv(co)%bio(2)
...
if ((ssv(ssp%cohort)%lai>0.1).and.(msv%mv_soil2g>ssp%wilt)) then
