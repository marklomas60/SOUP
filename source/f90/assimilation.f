* The subroutines are just copies of nppcalc sunshade and light
      implicit none
      real*8 rlai,lat,hrs,npp_eff,maxc,minn(3),p,oi,tmp,rh,cld
      real*8 fpr,nleaf,canga,gsum,amax,amx,suma,canres,can2g
      real*8 can2a,rd,rn,s,windspeed,qdirect,qdiff,ht,wtfc,wtwp
      real*8 soilc,soiln,ca,soil2g,dayl,q,lam,t
      integer c3,mnth,day,thty_dys,no_day,no_days,year,kode
      INCLUDE 'param.inc'
...
*     ht(ft) = 0.807d0*(laimax(ft)**2.13655d0)
      ht = 0.807d0*(5.0d0**2.13655d0)
     ...
     &0.7d0*ht)/(0.1d0*ht))**2
      ...
      CALL PFD(lat,no_day(year,mnth,day,thty_dys),hrs,cld,qdirect,
     &qdiff,q)
      t = tmp
     ...
     &rd,rlai,tmp,rh,ca,oi,rn,qdirect,qdiff,can2a,can2g,canres,suma,
     &amx,amax,gsum,hrs,canga/1.3d0,p,mnth,day,nleaf,fpr)
...
*        xx = (0.25 + (1.0d0-0.25d0)/25.0d0*t)**1.5d0
        ...
        if (t.gt.30.0d0) vmx(i)=100.0d0*qt**(30.0/10.0d0)/qt**(2.5d0)
