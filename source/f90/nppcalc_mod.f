...
*----------------------------------------------------------------------*
     ...
     &gsum,hrs,canga,p,mnth,day,nleaf_sum,fpr)
*----------------------------------------------------------------------*
...
c light-limited assimilation rate (j) and irradiance (q) for sunlit
c and shade
      REAL*8 jsunlit(12),jshade(12),qsunlit(12),qshade(12)
c fraction of sunlit and shade
...
c changed by Ghislain 08/10/03      IF (rlai.GT.0.1d0) THEN
      q = qdiff + qdirect
...
!        IF (t.GT.xx) THEN
!          jmx(i) = jm(i)*(1.0d0 + p_j1*(t-25.0yeard0) +
...
!        xx = (0.25 + (1.0d0-0.25d0)/25.0d0*t)**1.5d0
        ...
        vm(i) = nleaf*p_vm
        qt = 3.0
        vmx(i)=vm(i)*qt**(t/10.0)/qt**(2.5)
        if (t.gt.30.0d0) vmx(i)=100.0*qt**(30.0/10.0)/qt**(2.5)
        vmx(i) = vmx(i)*npp_eff*kg**p_nu3

!        vmx(i) = vmx(i)*(1.0d0+0.25d0*(1.0d0-ca/35.0d0))

        jmx(i) = (29.1d0 + 1.64*vmx(i))
        vmx(i) = vmx(i)*1.0e-6
        jmx(i) = jmx(i)*1.0e-6
...
c*********************************************************************
c     fAPAR calculation added here from gh nppcalc.f amf251105
c*********************************************************************
      CALL GOUDRIAANSLAW(rlai,rlai,qdirect,qdiff,
     &        fsunlit(lai+1),qsunlit(lai+1), fshade(lai+1),
     &        qshade(i))
...
c removed by Ghislain 24/11/03
c the output xvmx, xjmx and xj are not used anymore
c      CALL XVJMAX(xvmx,xjmx,xj,t,q,sum,nup,oi,xdresp)
...
c     make the sum of assimilation, internal concentration and stomatal
c     conductance.
              a(i)=0.0d0
              ci(i)=0.0d0
              gs(i)=0.0d0
...
c Temporary change by Ghislain
           ax=0
c end if change
...
c Mark we have to discuss about this calculation for irc here...
      irc = q*exp(-0.5d0)
